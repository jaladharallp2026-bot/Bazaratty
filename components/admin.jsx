...
import { Icon, Imagery, Stat, StatusPill, LineChart, BarChart, Donut, TopBar, Sidebar } from './shared';
...
function AdminShell({ active, go, children, title, search }) {
  ...
  return (
    <div className="app-shell">
      ...
      <Sidebar
        active={active} onPick={(id) => { go(id); setMenuOpen(false); }} items={ADMIN_NAV}
        open={menuOpen}
        headerExtra={
          <div className="card" style={{ margin: "10px 4px 14px", padding: 10 }}>
            <div className="row gap-10">
              <div style={{ width: 32, height: 32, borderRadius: 9, background: "var(--teal-400)", color: "var(--ink-900)", display: "flex", alignItems: "center", justifyContent: "center" }}>
...
export function AdminAnalytics({ go }) {
  const D = DATA;
  return (
    <AdminShell active="admin-dash" go={go} title="Platform analytics" search="Search users, sellers, orders…">
      <div className="row between" style={{ marginBottom: 22 }}>
        <div>
          ...
          <p className="muted" style={{ marginTop: 4 }}>14-day window · all amounts in KWD.</p>
        </div>
      ...
      <div className="dash-grid-4">
        <Stat label="GMV"             value="KD 284,910" delta="+18%" series={D.SERIES_REV.map(v => v*30)} />
      ...
      <div className="dash-grid-2" style={{ marginTop: 22 }}>
        <div className="card" style={{ padding: 22 }}>
          <div className="row between">
            <div><div className="strong">GMV (gross merchandise value)</div><div className="muted" style={{ fontSize: 12 }}>Daily, last 14 days</div></div>
            <span className="chip teal">+18% WoW</span>
          </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 18, marginTop: 18 }}>
        <div className="card" style={{ padding: 22 }}>
          ...
          <div className="col gap-10" style={{ marginTop: 14 }}>
            {[
              { n: "Maple & Oak",      v: "KD 12,418", g: "+18%" },
              { n: "Brewhaus",         v: "KD 9,820",  g: "+12%" },
              { n: "Verde Apothecary", v: "KD 8,640",  g: "+24%" },
              { n: "Lumen Studio",     v: "KD 7,210",  g: "+8%"  },
              { n: "Stride Lab",       v: "KD 6,180",  g: "+5%"  },
            ].map((s,i) => (
              <div key={s.n} className="row between">
                <div className="row gap-10">
                  <span className="mono muted" style={{ fontSize: 11, width: 18 }}>0{i+1}</span>
                  <span className="avatar" style={{ width: 26, height: 26, fontSize: 10 }}>{s.n.split(" ").map(x => x[0]).join("").slice(0,2)}</span>
                  <span style={{ fontSize: 13 }}>{s.n}</span>
                </div>
                <div className="row gap-10">
                  <span className="strong tnum" style={{ fontSize: 13 }}>{s.v}</span>
                  <span className="chip teal" style={{ fontSize: 11 }}>{s.g}</span>
                </div>
              </div>
            ))}
          </div>
  ...
  return (
    <AdminShell active="admin-users" go={go} title="Users" search="Search by name, email, role…">
      <div className="row between" style={{ marginBottom: 16 }}>
        ...
        <div className="row gap-8">
          {["All","Buyers","Sellers","Suspended"].map((t,i) => (
          ...
          <tbody>
            {users.map(u => (
              <tr key={u.e}>
                <td>
                  <div className="row gap-10">
                    <span className="avatar">{u.n.split(" ").map(s => s[0]).join("")}</span>
                    <div className="col"><span className="strong">{u.n}</span><span className="muted" style={{ fontSize: 11.5 }}>{u.e}</span></div>
                  </div>
                </td>
                ...
                <td>
                  <span className={"chip " + (u.s === "Active" ? "teal" : "rose")}>
                    <span style={{ width: 6, height: 6, borderRadius: 99, background: u.s === "Active" ? "var(--teal-500)" : "#c64a5b" }} />{u.s}
                  </span>
                </td>
...
export function AdminApprovals({ go }) {
  const A = DATA.APPROVALS;
  return (
    <AdminShell active="admin-approvals" go={go} title="Seller approvals">
      <div className="row between" style={{ marginBottom: 16 }}>
        <h1 style={{ fontSize: 26 }}>Seller approvals</h1>
        <span className="chip amber">{A.length} pending</span>
      </div>
...
export function AdminModeration({ go }) {
  const M = DATA.MODERATION;
  const hues = [30, 195, 350];
  return (
    <AdminShell active="admin-moderation" go={go} title="Product moderation">
      <div className="row between" style={{ marginBottom: 16 }}>
        <h1 style={{ fontSize: 26 }}>Product moderation</h1>
        <div className="row gap-8">
          {["Open","Reviewed","Closed"].map((t,i) => (
            <button key={t} className={"btn btn-sm " + (i === 0 ? "btn-primary" : "btn-ghost")}>{t}</button>
          ))}
        </div>
      </div>
      <div className="col gap-12">
        {M.map((m,i) => (
          <div key={m.id} className="card" style={{ padding: 18 }}>
            <div className="row between" style={{ alignItems: "flex-start" }}>
              <div className="row gap-14">
                <Imagery ratio="1/1" tone="tinted" hue={hues[i % hues.length]} radius={12} style={{ width: 76, height: 76 }} />
                <div className="col gap-4">
  ...
  return (
    <AdminShell active="admin-orders" go={go} title="Orders monitoring">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14 }}>
        <Stat label="Orders today"  value="324"  delta="+12%" series={DATA.SERIES_ORD.map(v => v*22)} />
...
export function AdminRevenue({ go }) {
  const D = DATA;
  return (
    <AdminShell active="admin-revenue" go={go} title="Revenue reports">
      <div className="row between" style={{ marginBottom: 18 }}>
        <h1 style={{ fontSize: 26 }}>Revenue reports</h1>
        <div className="row gap-8">
          <select className="input" style={{ height: 38, width: 150 }}><option>This quarter</option><option>This year</option></select>
          <button className="btn btn-ghost btn-sm">Download CSV</button>
        </div>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14 }}>
        <Stat label="Platform fee revenue" value="KD 17,094"  delta="+18%" series={D.SERIES_REV.map(v => v*1.8)} />
        <Stat label="GMV"                  value="KD 284,910" delta="+18%" series={D.SERIES_REV.map(v => v*30)} />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18, marginTop: 18 }}>
        <div className="card" style={{ padding: 22 }}>
          ...
          <div className="col gap-10" style={{ marginTop: 14 }}>
            {[
              { l: "Home & Living", v: "KD 108,260", p: 38 },
              { l: "Fashion",       v: "KD 68,378",  p: 24 },
              { l: "Beauty",        v: "KD 45,585",  p: 16 },
              { l: "Tech",          v: "KD 34,189",  p: 12 },
              { l: "Other",         v: "KD 28,491",  p: 10 },
            ].map(c => (
              <div key={c.l}>
        ...
        <div className="card" style={{ padding: 22 }}>
          ...
          <table className="tbl" style={{ marginTop: 8 }}>
            ...
            <tbody>
              {[
                { n: "Maple & Oak",      g: "KD 24,180", o: 320, t: "KD 1,451" },
                { n: "Verde Apothecary", g: "KD 22,640", o: 410, t: "KD 1,358" },
                { n: "Brewhaus",         g: "KD 19,820", o: 215, t: "KD 1,189" },
                { n: "Lumen Studio",     g: "KD 15,210", o: 142, t: "KD 913"   },
                { n: "Keycraft",         g: "KD 13,800", o:  88, t: "KD 828"   },
              ].map(s => (
                <tr key={s.n}>
...
export function AdminSettings({ go }) {
  return (
      ...
      <div style={{ display: "grid", gridTemplateColumns: "260px 1fr", gap: 18 }}>
        <aside className="col gap-4">
          {["General","Commissions","Payouts","Email & notifications","API & webhooks","Audit log"].map((s,i) => (
            <button key={s} className={"nav-link " + (i === 1 ? "active" : "")}>
              <Icon name="chev" size={14} /> <span className="grow" style={{ textAlign: "left" }}>{s}</span>
            </button>
          ))}
        </aside>
        <div className="col gap-18">
          <div className="card" style={{ padding: 22 }}>
            ...
            <table className="tbl">
              <thead><tr><th>Plan</th><th>Monthly</th><th>Take rate</th><th></th></tr></thead>
              <tbody>
                {[{ n: "Starter", m: "KD 0",  t: "8%" },
                  { n: "Studio",  m: "KD 9",  t: "6%" },
                  { n: "Scale",   m: "KD 30", t: "4%" }].map(p => (
                  <tr key={p.n}>
                    <td className="strong">{p.n}</td>
                    <td className="tnum">{p.m}</td>
                    <td className="tnum">{p.t}</td>
                    <td style={{ textAlign: "right" }}><button className="btn btn-ghost btn-sm"><Icon name="edit" size={13} /></button></td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="card" style={{ padding: 22 }}>
            ...
            <div className="row between" style={{ padding: "10px 0" }}>
              <div className="col"><span className="strong" style={{ fontSize: 13.5 }}>Auto-approve verified sellers</span><span className="muted" style={{ fontSize: 12 }}>Skip manual review for sellers with all docs complete.</span></div>
              <label className="row"><input type="checkbox" defaultChecked /></label>
            </div>
            <div className="divider" />
            <div className="row between" style={{ padding: "10px 0" }}>
              <div className="col"><span className="strong" style={{ fontSize: 13.5 }}>Hold first payout 7 days</span><span className="muted" style={{ fontSize: 12 }}>Reduce risk on new sellers' first orders.</span></div>
              <label className="row"><input type="checkbox" defaultChecked /></label>
            </div>
            <div className="divider" />
            <div className="row between" style={{ padding: "10px 0" }}>
              <div className="col"><span className="strong" style={{ fontSize: 13.5 }}>Allow international shipping by default</span><span className="muted" style={{ fontSize: 12 }}>New sellers ship globally unless they opt out.</span></div>
              <label className="row"><input type="checkbox" /></label>
            </div>
          </div>
        </div>
      </div>
