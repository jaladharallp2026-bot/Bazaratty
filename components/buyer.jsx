...
import { Icon, Imagery, Stat, Stars, StatusPill, TopBar, Sidebar, SectionHead, Empty } from './shared';
import DATA from './data';
import { ProductCard } from './public';
...
function BuyerShell({ active, go, children, title, search }) {
  ...
  return (
    <div className="app-shell">
      ...
      <Sidebar
        active={active}
        onPick={(id) => { go(id); setMenuOpen(false); }}
        items={BUYER_NAV}
        open={menuOpen}
        headerExtra={
...
export function BuyerOverview({ go }) {
  const D = DATA;
  return (
    <BuyerShell active="buyer-dash" go={go} title="Overview" search="Search orders, products, sellers…">
      ...
      <div className="dash-grid-4">
        ...
        <Stat label="Spent this year"   value="KD 1,284" delta="+12%" series={D.SERIES_REV} />
          ...
          <div className="col gap-12" style={{ marginTop: 14 }}>
            {D.PRODUCTS.slice(2, 5).map(p => (
              <button key={p.id} className="row gap-12 lift" style={{ padding: 8, borderRadius: 12, textAlign: 'left' }} onClick={() => go('pdp', { product: p })}>
                <Imagery ratio="1/1" tone="tinted" hue={(D.CATEGORIES.find(c => c.id === p.cat)||{}).hue || 162} radius={10} style={{ width: 56, height: 56 }} />
                ...
                <span className="strong tnum" style={{ fontSize: 13 }}>KD {p.price}</span>
              </button>
            ))}
          </div>
        </div>
...
export function BuyerOrders({ go }) {
  const D = DATA;
  const [filter, setFilter] = useState('all');
  const hues = [162, 220, 18, 350, 195];
    ...
    <BuyerShell active="buyer-orders" go={go} title="Orders" search="Search orders…">
      <div className="row between" style={{ marginBottom: 16 }}>
        ...
        <div className="row gap-8">
          {[{ id: 'all', l: 'All' }, { id: 'transit', l: 'In transit' }, { id: 'delivered', l: 'Delivered' }, { id: 'returned', l: 'Returned' }].map(t => (
        ...
        <table className="tbl">
          ...
          <tbody>
            {filtered.map((o, idx) => (
              <tr key={o.id}>
                <td className="mono tnum" style={{ fontSize: 12.5 }}>{o.id}<br/><span className="muted" style={{ fontSize: 11 }}>{o.date}</span></td>
                <td><div className="row gap-10"><Imagery ratio="1/1" tone="tinted" hue={hues[idx % hues.length]} radius={8} style={{ width: 36, height: 36 }} /><span className="strong">{o.product}</span></div></td>
                <td>{o.seller}</td>
                <td className="tnum strong">KD {o.total}</td>
                <td><StatusPill status={o.status} /></td>
                <td style={{ textAlign: 'right' }}><button className="btn btn-ghost btn-sm">Track</button></td>
              </tr>
            ))}
          </tbody>
        </table>
...
export function BuyerWishlist({ go }) {
...
export function BuyerMessages({ go }) {
      ...
      <div className="card" style={{ overflow: 'hidden', display: 'grid', gridTemplateColumns: '300px 1fr', height: 560 }}>
        ...
        <div className="col" style={{ height: '100%' }}>
          ...
          <div style={{ padding: 14, borderTop: '1px solid var(--ink-100)' }}>
            <div className="row gap-8"><input className="input grow" placeholder="Write a message…" /><button className="btn btn-primary">Send</button></div>
          </div>
        </div>
      </div>
    </BuyerShell>
...
export function BuyerNotifs({ go }) {
...
export function BuyerProfile({ go }) {
  return (
