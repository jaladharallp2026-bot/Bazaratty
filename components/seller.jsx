...
import { Icon, Logo, Imagery, Stat, StatusPill, LineChart, BarChart, Donut, TopBar, Sidebar } from './shared';
...
function SellerShell({ active, go, children, title, search }) {
  ...
  return (
    <div className="app-shell">
      {menuOpen && <div className="mob-overlay show" onClick={() => setMenuOpen(false)} />}
      <Sidebar
        active={active} onPick={(id) => { go(id); setMenuOpen(false); }} items={SELLER_NAV}
        open={menuOpen}
        headerExtra={
      ...
      />
      <main>
        <TopBar title={title} search={search} onMenuToggle={() => setMenuOpen(o => !o)} right={
          <>
...
export function SellerAnalytics({ go }) {
  const D = DATA;
  return (
    <SellerShell active="seller-dash" go={go} title="Analytics" search="Search products, orders, customers…">
      ...
      <div className="dash-grid-4">
        <Stat label="Revenue"    value="KD 12,418" delta="+18%" series={D.SERIES_REV} />
      ...
      <div className="dash-grid-2" style={{ marginTop: 22 }}>
        <div className="card" style={{ padding: 22 }}>
          <div className="row between">
            <div><div className="strong">Revenue trend</div><div className="muted" style={{ fontSize: 12 }}>KWD · last 14 days</div></div>
        ...
        <div className="card" style={{ padding: 22 }}>
          ...
          <div className="row" style={{ marginTop: 14, alignItems: "center", gap: 18 }}>
            <Donut
              ...
              label={{ value: "KD 12.4k", sub: "REVENUE" }}
            />
            <div className="col gap-10 grow">
              {[
                ...
                <div key={x.l} className="row between">
                  <div className="row gap-8"><span style={{ width: 8, height: 8, borderRadius: 99, background: x.c }} /><span style={{ fontSize: 13 }}>{x.l}</span></div>
                  <span className="strong tnum" style={{ fontSize: 13 }}>{x.v}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18, marginTop: 18 }}>
        <div className="card" style={{ padding: 22 }}>
          <div className="row between">
            <div className="strong">Orders per day</div>
            <a className="muted" style={{ fontSize: 12, cursor: "pointer" }} onClick={() => go("seller-orders")}>View orders</a>
          </div>
            ...
            {D.SELLER_PRODUCTS.slice(0,4).map((p,i) => (
              <div key={p.id} className="row gap-12">
                ...
                <div className="col gap-4 grow">
                  <span className="strong" style={{ fontSize: 13 }}>{p.name}</span>
...
export function SellerProducts({ go }) {
  const D = DATA.SELLER_PRODUCTS;
  return (
          ...
          <tbody>
            {D.map((p, i) => (
              <tr key={p.id}>
                ...
                <td>
                  <div className="row gap-10">
                    <Imagery ratio="1/1" tone="tinted" hue={162 + i*22} radius={8} style={{ width: 40, height: 40 }} />
                    <div className="col"><span className="strong">{p.name}</span><span className="muted" style={{ fontSize: 11 }}>Home &amp; Living</span></div>
                  </div>
                </td>
                <td className="mono muted" style={{ fontSize: 12.5 }}>{p.id}</td>
                <td className="tnum strong">KD {p.price}</td>
...
export function SellerAddProduct({ go }) {
  return (
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.5fr 1fr", gap: 18 }}>
        <div className="col gap-18">
          <div className="card" style={{ padding: 22 }}>
            ...
            <input className="input" placeholder="e.g. Walnut wall shelf set" />
            <label className="label" style={{ marginTop: 12 }}>Description</label>
            <textarea className="input" rows="4" placeholder="Tell buyers what makes it special…"></textarea>
            <div className="row gap-12" style={{ marginTop: 12 }}>
              <div className="grow">
                <label className="label">Category</label>
                <select className="input">{DATA.CATEGORIES.map(c => <option key={c.id}>{c.name}</option>)}</select>
              </div>
          ...
          <div className="card" style={{ padding: 22 }}>
            ...
            <div className="divider" style={{ margin: "12px 0 18px" }} />
            <table className="tbl">
              <thead><tr><th>Variant</th><th>Price</th><th>Stock</th><th>SKU</th></tr></thead>
              <tbody>
                {[{v: "Natural / Small", p: 118, s: 12, k: "WS-NS"}, {v: "Natural / Large", p: 168, s: 8, k: "WS-NL"}, {v: "Black / Small", p: 128, s: 14, k: "WS-BS"}].map(r => (
                  <tr key={r.k}><td className="strong">{r.v}</td><td className="tnum">KD {r.p}</td><td className="tnum">{r.s}</td><td className="mono muted" style={{ fontSize: 12 }}>{r.k}</td></tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="col gap-18">
          <div className="card" style={{ padding: 22 }}>
            ...
            <div className="row gap-12">
              <div className="grow"><label className="label">Price</label><input className="input" placeholder="KD 0.000" defaultValue="KD 118" /></div>
              <div className="grow"><label className="label">Compare at</label><input className="input" placeholder="KD 0.000" /></div>
            </div>
...
export function SellerOrders({ go }) {
  const D = DATA.ORDERS_SELLER;
  return (
    <SellerShell active="seller-orders" go={go} title="Orders" search="Search by order, customer, product…">
      <div className="row between" style={{ marginBottom: 16 }}>
        ...
        <div className="row gap-8">
          {["All","Awaiting ship","Shipped","Delivered","Refunded"].map((t,i) => (
            <button key={t} className={"btn btn-sm " + (i === 1 ? "btn-primary" : "btn-ghost")}>{t}</button>
          ))}
        </div>
      </div>
      <div className="card" style={{ overflow: "hidden" }}>
          ...
          <tbody>
            {D.map(o => (
              <tr key={o.id}>
                ...
                <td>
                  <div className="row gap-10">
                    <span className="avatar">{o.buyer.split(" ").map(s => s[0]).join("")}</span>
                    <span className="strong">{o.buyer}</span>
                  </div>
                </td>
                <td>{o.product}</td>
                <td className="tnum">{o.qty}</td>
                <td className="tnum strong">KD {o.total}</td>
...
export function SellerRevenue({ go }) {
  const D = DATA;
  return (
    <SellerShell active="seller-revenue" go={go} title="Revenue">
      <div className="row between" style={{ marginBottom: 18 }}>
        <h1 style={{ fontSize: 26 }}>Revenue</h1>
        <div className="row gap-8">
          <select className="input" style={{ height: 38, width: 140 }}><option>This quarter</option><option>This year</option></select>
        </div>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14 }}>
        <Stat label="Gross revenue" value="KD 38,920" delta="+22%" series={D.SERIES_REV} />
        <Stat label="Net payout"    value="KD 36,587" delta="+21%" series={D.SERIES_REV.map(v => v*0.94)} />
        <Stat label="Avg order"     value="KD 87.420" delta="+3%"  series={[78,80,82,81,82,84,85,84,86,86,87,87,87,87.4]} />
        <Stat label="Refunds"       value="KD 420"    delta="-12%" series={[80,75,70,68,60,55,52,50,48,45,42,40,42,42]} />
      </div>
      ...
      <div className="card" style={{ marginTop: 18, padding: 22 }}>
        <div className="row between">
          <div className="strong">Recent payouts</div>
          <a className="muted" style={{ fontSize: 12, cursor: "pointer" }}>View all</a>
        </div>
        <table className="tbl" style={{ marginTop: 8 }}>
          <thead><tr><th>Reference</th><th>Date</th><th>Method</th><th>Amount</th><th>Status</th></tr></thead>
          <tbody>
            {[
              { ref: "PAY-3041", d: "May 8", m: "Bank transfer · ··15 4", a: "KD 2,148", s: "Complete" },
              { ref: "PAY-3032", d: "May 6", m: "Bank transfer · ··15 4", a: "KD 1,892", s: "Complete" },
              { ref: "PAY-3018", d: "May 4", m: "Bank transfer · ··15 4", a: "KD 2,560", s: "Complete" },
              { ref: "PAY-3007", d: "May 2", m: "Bank transfer · ··15 4", a: "KD 1,420", s: "Complete" },
            ].map(p => (
              <tr key={p.ref}>
...
export function SellerStorefront({ go }) {
  ...
  return (
    <SellerShell active="seller-store" go={go} title="Storefront">
      <div className="row between" style={{ marginBottom: 18 }}>
        <h1 style={{ fontSize: 26 }}>Customize storefront</h1>
        <div className="row gap-10">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "320px 1fr", gap: 18 }}>
        <div className="card" style={{ padding: 22, alignSelf: "flex-start" }}>
          ...
          <div className="col gap-8">
            {[
              { id: "editorial", l: "Editorial — large hero" },
              { id: "grid",      l: "Grid — product first" },
              { id: "lookbook",  l: "Lookbook — long-scroll" },
            ].map(o => (
              <label key={o.id} className="row gap-8 card" style={{ padding: 10, cursor: "pointer", border: layout === o.id ? "1.5px solid var(--ink-900)" : "1px solid var(--ink-100)" }}>
                <input type="radio" checked={layout === o.id} onChange={() => setLayout(o.id)} />
        ...
        <div className="card" style={{ padding: 0, overflow: "hidden" }}>
          <div style={{ padding: "10px 14px", borderBottom: "1px solid var(--ink-100)", background: "var(--ink-50)" }} className="row gap-6">
            ...
            <span style={{ width: 10, height: 10, borderRadius: 99, background: "#f5bf4f" }} />
          ...
          <div style={{ padding: 28 }}>
            <div className="row between">
              <Logo size={22} />
              <div className="row gap-16 muted" style={{ fontSize: 12 }}><span>Shop</span><span>Story</span><span>Contact</span></div>
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14, marginTop: 22 }}>
              {DATA.SELLER_PRODUCTS.slice(0,3).map((p, i) => (
                <div key={p.id} className="card" style={{ padding: 10 }}>
                  ...
                  <div className="row between" style={{ marginTop: 4 }}>
                    <span className="muted" style={{ fontSize: 12 }}>Maple &amp; Oak</span>
                    <span className="strong tnum">KD {p.price}</span>
                  </div>
                </div>
              ))}
            </div>
...
export function SellerCustomerMessages({ go }) {
  return (
        ...
        <div style={{ borderRight: "1px solid var(--ink-100)", overflow: "auto" }}>
          {[
            ...
            <div key={i} style={{ padding: "14px 16px", borderBottom: "1px solid var(--ink-100)", background: i === 0 ? "var(--ink-50)" : "transparent" }}>
              <div className="row between">
                <span className="strong" style={{ fontSize: 13 }}>{m.who}</span>
        ...
        <div className="col" style={{ height: "100%" }}>
          <div className="row between" style={{ padding: 16, borderBottom: "1px solid var(--ink-100)" }}>
            <div className="row gap-10">
              ...
              <div className="col"><span className="strong" style={{ fontSize: 13 }}>M. Khan</span><span className="muted" style={{ fontSize: 11 }}>Order ORD-7821</span></div>
            </div>
          ...
          <div style={{ padding: 14, borderTop: "1px solid var(--ink-100)" }}>
            <div className="row gap-8"><input className="input grow" placeholder="Reply…" /><button className="btn btn-primary">Send</button></div>
          </div>
        </div>
      </div>
    </SellerShell>
...
export function SellerPayouts({ go }) {
  return (
        ...
        <div className="col gap-18">
          <div className="card" style={{ padding: 22, background: "var(--teal-50)", borderColor: "var(--teal-100)" }}>
            ...
            <div className="row between" style={{ alignItems: "baseline", marginTop: 10 }}>
              <span className="tnum" style={{ fontFamily: "var(--display)", fontWeight: 700, fontSize: 28 }}>KD 2,418</span>
