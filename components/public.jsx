...
import { Logo, Icon, Imagery, Stars, SectionHead, Reveal, Counter, Sparkline, Empty } from './shared';
...
export function ProductCard({ p, go }) {
  const [fav, setFav] = useState(false);
  const photo = DATA.PRODUCT_PHOTO[p.id];
  const cat = DATA.CATEGORIES.find(c => c.id === p.cat) || { hue: 162 };
  return (
    <div className="card lift" style={{ padding: 0, cursor: 'pointer', overflow: 'hidden', borderRadius: 16 }} onClick={() => go('pdp', { product: p })}>
      <div className="zoomy" style={{ position: 'relative', borderRadius: 0 }}>
        ...
        <button
          ...
          style={{ position: 'absolute', top: 12, right: 12, zIndex: 2, width: 34, height: 34, borderRadius: 999, background: 'rgba(255,255,255,.96)', border: '1px solid rgba(0,0,0,.06)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: fav ? '#c64a5b' : 'var(--ink-700)', backdropFilter: 'blur(6px)', boxShadow: '0 2px 8px rgba(0,0,0,.06)' }}
          aria-label="Favorite"
        >
      ...
      <div style={{ padding: '14px 14px 16px' }}>
        <div className="row between" style={{ alignItems: 'flex-start', gap: 10 }}>
          ...
          <span className="tnum" style={{ fontFamily: 'var(--sans)', fontWeight: 600, fontSize: 14 }}>KD {p.price}</span>
        </div>
...
export function PublicNav({ go, active }) {
  const [navOpen, setNavOpen] = useState(false);
  const link = (id, label) => (
    <a onClick={() => { go(id); setNavOpen(false); }} style={{ cursor: 'pointer', color: active === id ? 'var(--ink-900)' : 'var(--ink-600)', fontWeight: active === id ? 600 : 500 }}>{label}</a>
  );
  return (
    <>
      <div className="row between" style={{ padding: '16px 24px', borderBottom: '1px solid var(--ink-100)', background: 'rgba(255,255,255,.85)', backdropFilter: 'blur(8px)', position: 'sticky', top: 0, zIndex: 20 }}>
        <div className="row gap-24">
          <Logo size={28} variant="chip" />
          <div className="pub-nav-links" style={{ fontSize: 13.5, gap: 20 }}>
        ...
        <div className="row gap-8">
          <div className="pub-nav-links" style={{ position: 'relative', width: 300 }}>
            <span style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: 'var(--ink-400)' }}><Icon name="search" size={16} /></span>
            <input className="input" placeholder="Search products…" style={{ paddingLeft: 36, height: 38 }} onClick={() => go('search')} readOnly />
          </div>
          <button className="pub-nav-links btn btn-ghost btn-sm" aria-label="wishlist"><Icon name="heart" size={15} /></button>
          <button className="pub-nav-links btn btn-ghost btn-sm" aria-label="cart"><Icon name="cart" size={15} /></button>
          <button className="btn btn-primary btn-sm" onClick={() => go('login')}>Sign in</button>
          <button className="pub-hamburger btn btn-ghost btn-sm" onClick={() => setNavOpen(true)} aria-label="menu"><Icon name="menu" size={18} /></button>
        </div>
      </div>
      <div className={'mob-nav-drawer ' + (navOpen ? 'show' : '')}>
        <div className="row between" style={{ marginBottom: 24 }}>
          <Logo size={28} />
          <button className="btn btn-ghost btn-sm" onClick={() => setNavOpen(false)}><Icon name="x" size={18} /></button>
        </div>
        <div className="col gap-4">
          {[['market','Marketplace'],['categories','Categories'],['about','About'],['pricing','Pricing'],['contact','Contact']].map(([id, label]) => (
            <button key={id} className={'nav-link' + (active === id ? ' active' : '')} onClick={() => { go(id); setNavOpen(false); }} style={{ fontSize: 15, padding: '12px 14px' }}>{label}</button>
          ))}
        </div>
        <div style={{ flex: 1 }} />
        <div className="col gap-8" style={{ paddingTop: 20, borderTop: '1px solid var(--ink-100)' }}>
          <button className="btn btn-ghost" style={{ justifyContent: 'center' }} onClick={() => { go('login'); setNavOpen(false); }}>Sign in</button>
          <button className="btn btn-primary" style={{ justifyContent: 'center' }} onClick={() => { go('signup'); setNavOpen(false); }}>Get started</button>
        </div>
      </div>
    </>
  );
...
export function Landing({ go }) {
  ...
  return (
    <div className="fadein">
      <div className="row between" style={{ padding: '16px 28px', borderBottom: '1px solid var(--ink-100)', background: 'rgba(245,250,248,.85)', backdropFilter: 'blur(10px)', position: 'sticky', top: 0, zIndex: 20 }}>
        <Logo size={32} variant="chip" />
        <div className="pub-nav-links" style={{ fontSize: 13.5, color: 'var(--ink-600)' }}>
        ...
        <div className="row gap-8">
          <button className="pub-nav-links btn btn-ghost btn-sm" onClick={() => go('login')}>Sign in</button>
          <button className="pub-nav-links btn btn-primary btn-sm" onClick={() => go('signup')}>Get started <Icon name="arrow" size={14} /></button>
          <button className="pub-hamburger btn btn-ghost btn-sm" onClick={() => setNavOpen(true)}><Icon name="menu" size={18} /></button>
        </div>
      </div>
      <div className={'mob-nav-drawer ' + (navOpen ? 'show' : '')}>
        <div className="row between" style={{ marginBottom: 24 }}>
        ...
        <div className="col gap-4">
          {[['market','Marketplace'],['categories','Categories'],['about','About'],['pricing','Pricing'],['contact','Contact']].map(([id, label]) => (
            <button key={id} className="nav-link" onClick={() => { go(id); setNavOpen(false); }} style={{ fontSize: 15, padding: '12px 14px' }}>{label}</button>
          ))}
        </div>
        <div style={{ flex: 1 }} />
        <div className="col gap-8" style={{ paddingTop: 20, borderTop: '1px solid var(--ink-100)' }}>
          ...
          <div className="hero-img-col" style={{ position: 'relative', height: 580 }}>
            ...
            <div className="card" style={{ position: 'absolute', left: -8, top: 40, padding: 14, width: 240, boxShadow: 'var(--shadow-lg)', background: 'white', borderRadius: 16 }}>
              ...
              <div className="row between" style={{ marginTop: 10, alignItems: 'center' }}>
                <span style={{ fontFamily: 'var(--sans)', fontWeight: 600 }} className="tnum">KD 92</span>
      ...
      <section style={{ padding: '96px 48px 56px', maxWidth: 1320, margin: '0 auto' }}>
        <Reveal>
      ...
      <section style={{ padding: '96px 0', margin: '48px 0', background: 'var(--teal-50)', borderTop: '1px solid var(--teal-100)', borderBottom: '1px solid var(--teal-100)' }}>
        <div className="split-img-text" style={{ maxWidth: 1320, margin: '0 auto', padding: '0 40px' }}>
          ...
          <div>
            ...
            <div className="split-1-1" style={{ marginTop: 36 }}>
              {[
                ...
                <div key={c.n} style={{ borderTop: '1px solid var(--teal-100)', paddingTop: 16 }}>
          ...
          {cats.map((c, i) => (
            <Reveal key={c.id} delay={i * 40}>
              <button onClick={() => go('market')} className="card lift zoomy" style={{ padding: 0, textAlign: 'left', overflow: 'hidden', borderRadius: 16, width: '100%' }}>
                <Imagery ratio="1/1" tone="tinted" hue={c.hue} radius={0} src={DATA.CATEGORY_PHOTO[c.id]} />
              ...
              <div>
                <div className="dash-grid-3 seller-stats" style={{ marginBottom: 40 }}>
                  {[
                    ...
                    { v: '4.9',  l: 'Avg seller rating',    s: '1,200+ reviews' },
                  ].map((s, i) => (
                    <div key={i} style={{ paddingLeft: i === 0 ? 0 : 28, borderLeft: i > 0 ? '1px solid rgba(255,255,255,.1)' : 'none' }}>
                      <div style={{ fontFamily: 'var(--display)', fontSize: 48, lineHeight: 1, color: 'var(--teal-300)' }}>{s.v}</div>
                      <div style={{ fontSize: 12.5, color: 'rgba(255,255,255,.65)', marginTop: 8 }}>{s.l}</div>
                      <div style={{ fontSize: 11.5, color: 'var(--teal-400)', marginTop: 4 }}>{s.s}</div>
                    </div>
                  ))}
                </div>
                <div style={{ background: 'rgba(255,255,255,.04)', borderRadius: 14, padding: '14px 20px', border: '1px solid rgba(255,255,255,.06)', overflow: 'hidden' }}>
                  ...
                  <Sparkline data={D.SERIES_REV} color="#84d6b8" w={520} h={60} />
                </div>
              </div>
              <div className="col gap-18">
                <div style={{ borderRadius: 18, overflow: 'hidden', height: 240 }}>
                  <img src={D.PHOTOS.store} alt="Seller storefront" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                </div>
                <div>
                  <div style={{ fontFamily: 'var(--display)', fontSize: 22, lineHeight: 1.3, marginBottom: 16 }}>Open your store in minutes. Get paid in 2 days.</div>
                  <button className="btn btn-teal btn-lg" onClick={() => go('seller-onboarding')} style={{ width: '100%', justifyContent: 'center' }}>Start selling <Icon name="arrow" size={16} /></button>
                </div>
              </div>
            </div>
          </div>
          ...
          {[
            ...
            <Reveal key={i} delay={i * 80}>
              <div className="card" style={{ padding: 28, height: '100%', borderRadius: 20, background: i === 1 ? 'var(--ink-900)' : 'white', color: i === 1 ? 'white' : 'var(--ink-900)', borderColor: i === 1 ? 'var(--ink-900)' : 'var(--ink-100)' }}>
                ...
                <p style={{ marginTop: 10, fontFamily: 'var(--display)', fontSize: 22, lineHeight: 1.3, letterSpacing: '-0.01em' }}>{t.q}</p>
      ...
      <section style={{ padding: '64px 40px 96px', maxWidth: 1320, margin: '0 auto' }}>
        <div style={{ padding: '80px 64px', background: 'var(--ink-900)', color: 'white', borderRadius: 32, position: 'relative', overflow: 'hidden', textAlign: 'center' }}>
          <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse at 65% 0%, rgba(88,195,163,.38), transparent 55%), radial-gradient(ellipse at 15% 85%, rgba(88,195,163,.12), transparent 50%)' }} />
          <div style={{ position: 'relative' }}>
            <div className="eyebrow" style={{ color: 'var(--teal-300)', marginBottom: 18 }}>Ready when you are</div>
            <h2 style={{ color: 'white', fontSize: 'clamp(48px, 6vw, 76px)', lineHeight: 1, letterSpacing: '-0.03em', maxWidth: 900, margin: '0 auto' }}>
...
export function Marketplace({ go }) {
  const D = DATA;
  return (
    <div className="fadein">
      <PublicNav go={go} active="market" />
      <div style={{ padding: '32px 40px 0', maxWidth: 1280, margin: '0 auto' }}>
        <div className="card split-3-2" style={{ padding: 28, background: 'var(--teal-50)', borderColor: 'var(--teal-100)' }}>
          <div className="col gap-12" style={{ justifyContent: 'center' }}>
            ...
            <p className="muted" style={{ maxWidth: 480 }}>Curated picks from independent makers. Free shipping over KD 80.</p>
      ...
      <section style={{ padding: '32px 40px', maxWidth: 1280, margin: '0 auto' }}>
        ...
        <div className="pub-grid-4" style={{ marginTop: 22 }}>
...
export function CategoriesPage({ go }) {
  return (
    <div className="fadein">
      ...
      <section style={{ padding: '8px 40px 56px', maxWidth: 1280, margin: '0 auto' }}>
        <div className="pub-grid-4">
...
export function SearchPage({ go }) {
  ...
  const filtered = useMemo(() => {
    ...
    return xs;
  }, [query, cat, priceMax, sort, D.PRODUCTS]);
      ...
      <section style={{ padding: '32px 40px', maxWidth: 1280, margin: '0 auto' }}>
        ...
        <div className="search-grid">
          <aside className="card" style={{ padding: 18, alignSelf: 'flex-start', position: 'sticky', top: 80 }}>
            ...
            <div style={{ marginTop: 18 }}>
              <div className="row between"><span className="label" style={{ marginBottom: 0 }}>Max price</span><span className="mono tnum" style={{ fontSize: 12 }}>KD {priceMax}</span></div>
...
export function ProductDetail({ go, product }) {
  const p = product || DATA.PRODUCTS[0];
  const cat = DATA.CATEGORIES.find(c => c.id === p.cat) || { hue: 162 };
  const [activeImg, setActiveImg] = useState(0);
  const [qty, setQty] = useState(1);
  const [tab, setTab] = useState('description');
  const related = DATA.PRODUCTS.filter(x => x.id !== p.id).slice(0, 4);
  return (
    <div className="fadein">
      <PublicNav go={go} active="market" />
      <section style={{ padding: '20px 40px 0', maxWidth: 1280, margin: '0 auto' }}>
        <div className="row gap-6" style={{ fontSize: 12.5, color: 'var(--ink-500)' }}>
          ...
          <a onClick={() => go('categories')} style={{ cursor: 'pointer' }}>{(DATA.CATEGORIES.find(c => c.id === p.cat) || {}).name}</a>
          <Icon name="chev" size={12} />
          <span className="strong">{p.title}</span>
        </div>
      </section>
      <section style={{ padding: '20px 40px 32px', maxWidth: 1280, margin: '0 auto' }}>
        ...
        <div className="col gap-12">
          <div className="row gap-10">
            <span className="chip teal">{(DATA.CATEGORIES.find(c => c.id === p.cat) || {}).name}</span>
          ...
          <div className="row gap-12" style={{ alignItems: 'baseline', marginTop: 6 }}>
            <span className="tnum" style={{ fontFamily: 'var(--display)', fontWeight: 700, fontSize: 36, letterSpacing: '-0.02em' }}>KD {p.price}</span>
            <span className="muted" style={{ textDecoration: 'line-through' }}>KD {(p.price * 1.18).toFixed(0)}</span>
          ...
          <div className="row gap-10" style={{ marginTop: 4 }}>
            ...
            <button className="btn btn-teal btn-lg grow" style={{ justifyContent: 'center' }}><Icon name="cart" size={16} /> Add to cart · KD {p.price * qty}</button>
          ...
          <div className="row gap-16" style={{ color: 'var(--ink-500)', fontSize: 12.5, marginTop: 6 }}>
            <span className="row gap-6"><Icon name="truck" size={14} /> Free shipping over KD 80</span>
...
export function ContactPage({ go }) {
  return (
    <div className="fadein">
      ...
      <section style={{ padding: '56px 40px', maxWidth: 960, margin: '0 auto' }}>
        ...
        <div className="card" style={{ padding: 22 }}>
          <div className="label">Name</div><input className="input" placeholder="Your name" />
          <div className="label" style={{ marginTop: 12 }}>Email</div><input className="input" placeholder="you@example.com" />
...
export function PricingPage({ go }) {
  const tiers = [
    { name: 'Starter', price: 'KD 0',   per: '/month', best: false, fee: '8% per sale', lines: ['Up to 25 listings', 'Standard storefront', '2-day payouts', 'Email support'] },
    { name: 'Studio',  price: 'KD 9',   per: '/month', best: true,  fee: '6% per sale', lines: ['Unlimited listings', 'Custom storefront', 'Priority payouts', 'Promo tools', 'Live chat support'] },
    { name: 'Scale',   price: 'KD 30',  per: '/month', best: false, fee: '4% per sale', lines: ['Multi-store', 'Wholesale tools', 'API access', 'Dedicated manager'] },
  ];
  return (
    <div className="fadein">
