...
import { Logo, Icon, Imagery, Stars } from './shared';
        ...
        {[
          ...
          >
            <div className="row between">
              <span style={{ width: 32, height: 32, borderRadius: 10, background: 'var(--teal-50)', color: 'var(--teal-700)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Icon name={r.icon} size={16} /></span>
...
export function SellerOnboarding({ go }) {
        ...
        <aside className="col gap-6">
          {steps.map((s, i) => (
            <div key={i} className="row gap-12" style={{ padding: '10px 12px', borderRadius: 10, background: i === step ? 'var(--ink-50)' : 'transparent' }}>
              <span style={{ width: 26, height: 26, borderRadius: 99, fontSize: 12, fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', background: i < step ? 'var(--teal-400)' : i === step ? 'var(--ink-900)' : 'var(--ink-100)', color: i <= step ? 'white' : 'var(--ink-500)' }}>
              ...
              <div className="col gap-14">
                ...
                <div><label className="label">What do you sell?</label>
                  <div className="row gap-8" style={{ flexWrap: 'wrap' }}>
                    {DATA.CATEGORIES.map(c => <button key={c.id} className="chip" style={{ cursor: 'pointer' }}>{c.name}</button>)}
                  </div>
                </div>
                ...
                <div className="row gap-12">
                  <div className="grow"><label className="label">Country</label><select className="input"><option>Kuwait</option><option>Saudi Arabia</option><option>UAE</option><option>Other</option></select></div>
                  <div className="grow"><label className="label">Currency</label><select className="input"><option>KWD (KD)</option><option>SAR (SR)</option><option>AED (AED)</option><option>USD $</option></select></div>
                </div>
              </div>
            )}
            {step === 1 && (
              <div className="col gap-14">
                <div className="row gap-12">
                  ...
                  <div style={{ width: 140 }}><label className="label">Price (KD)</label><input className="input" defaultValue="KD 118" /></div>
                </div>
