// Shared atoms + chrome for Bazaratty.
const { useState, useEffect, useRef, useMemo, useCallback } = React;

// ---------- Logo ----------
// Uses the actual brand mark image (white "b" on teal square).
...
function Logo({ size = 32, mark = false, color, variant = "auto" }) {
  // variant: "auto" / "light" / "chip" — all render the same brand image; only the wordmark color changes.
...
// ---------- Imagery ----------
// Supports real photo URLs via `src`; falls back to a tasteful tinted placeholder.
function Imagery({ ratio = "4/3", label, hue = 162, tone = "soft", radius = 14, src, alt = "", style, overlay }) {
...
// ---------- Sparkline ----------
function Sparkline({ data, color = "var(--teal-500)", w = 120, h = 36, fill = true }) {
...
// ---------- Bar chart (simple) ----------
function BarChart({ data, w = 560, h = 200, color = "var(--teal-400)" }) {
...
function LineChart({ series, w = 720, h = 240, color = "var(--teal-500)" }) {
...
// ---------- Donut ----------
function Donut({ slices, size = 132, thickness = 18, label }) {
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      ...
      {label && (
        <g>
          <text x={c} y={c - 2} textAnchor="middle" fontSize="20" fontFamily="var(--display)" fontWeight="700" fill="var(--ink-900)">
            {label.value}
          </text>
          <text x={c} y={c + 16} textAnchor="middle" fontSize="11" fill="var(--ink-500)" letterSpacing=".04em">
            {label.sub}
          </text>
        </g>
      )}
...
// ---------- Stat card ----------
function Stat({ label, value, delta, series, accent = "var(--teal-500)" }) {
...
// ---------- TopBar ----------
function TopBar({ title, search, right, onMenu }) {
  return (
    <div className="topbar">
      <div className="row gap-12 grow" style={{ minWidth: 0 }}>
        ...
        {search && (
          <div className="row" style={{ marginLeft: 18, flex: 1, maxWidth: 520, position: "relative" }}>
...
// ---------- Sidebar ----------
function Sidebar({ items, active, onPick, footer, headerExtra }) {
  return (
...
// ---------- Empty state ----------
function Empty({ icon = "package", title, body, action }) {
  return (
...
Object.assign(window, {
  Logo, Icon, Imagery, Sparkline, BarChart, LineChart, Donut, Stat, Stars,
  StatusPill, TopBar, Sidebar, Empty, SectionHead, Counter, Reveal,
});
